...
def handle_resolve_shortcut(ack, shortcut, say, client):
    ...
    try:
        ...
        participants = {
            msg["user"] for msg in replies["messages"][1:] if "bot_id" not in msg
        }

        user_options = [{
            "text": {"type": "plain_text", "text": f"<@{p_id}>"},
            "value": p_id
        } for p_id in participants]
        
        user_options.append({
            "text": {"type": "plain_text", "text": "No one / Self-solved"},
            "value": "self_solved"
        })
            ...
            view={
                ...
                "blocks": [
                    ...
                    },
                    {
                        ...
                        "element": { "type": "multi_static_select", "action_id": "contributors_select", "options": user_options },
                        "optional": True,
                    },
                ],
            },
